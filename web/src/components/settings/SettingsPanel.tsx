  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className={s.chart} aria-hidden>
      {buckets.map((b, i) => {
        ...
        const y = H - PAD - barH;
        return (
          <rect key={b.date} x={x} y={y} width={barW} height={barH}
            className={b.total > 0 ? s.barActive : s.barEmpty} rx={1}>
...
export function SettingsPanel({ accessToken, onBack }: Props) {
  ...
  useEffect(() => {
    fetch('/api/user/settings', { headers: { Authorization: `Bearer ${accessToken}` } })
      .then(r => r.json())
      .then(json => {
  ...
  useEffect(() => {
    fetch('/api/generate/credit', { headers: { Authorization: `Bearer ${accessToken}` }, cache: 'no-store' })
      .then(r => r.json())
      .then(json => { const r = Number(json?.data); if (Number.isFinite(r)) setCreditsRemaining(r); })
      .catch(() => {});
  }, [accessToken]);

  useEffect(() => {
    setUsageLoading(true);
    fetch('/api/user/credit-usage', { headers: { Authorization: `Bearer ${accessToken}` } })
      .then(r => r.json())
  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.body}>
              ...
              <div className={s.keyRow}>
                <div className={s.keyInputWrap}>
                  <input
                    ...
                    autoComplete="off"
                  />
                  <button type="button" className={s.keyToggle}
                    onClick={() => setShowKey(v => !v)}
                    aria-label={showKey ? 'Hide key' : 'Show key'}>
                    {showKey ? <EyeOff size={14} /> : <Eye size={14} />}
                  </button>
                </div>
                <button className="btn primary sm" onClick={handleSaveKey} disabled={keyLoading}>
                  {keyLoading ? 'Saving…' : 'Save'}
                </button>
              </div>
            </div>
            <div className="hint">Keys are stored securely per account. Leave blank to use the server default key.</div>
          </div>
        </div>
        ...
        <div className={s.panel}>
          <div className={s.panelHead}>
            ...
            <div className={s.panelDesc}>Credit balance and 30-day usage trend for your account.</div>
          </div>
          <div className={s.panelBody}>
      ...
      {toast && (
        <div className={`${s.toast}${toast.err ? ` ${s.err}` : ''}`}>{toast.msg}</div>
      )}
    </div>
  );
}
